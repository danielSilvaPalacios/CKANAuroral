    ...
    for sub in oids_sub:  # Using set to obtain unique values
        ...
        for triple in filtered_results:
            ...
            if predicate == 'https://www.w3.org/2019/wot/td#serviceDescription':  # DESCRIPTION
                ...
                dataset_data['groups'][0]['name'] = process_string(
                    object_value)
...
def get_ckan_datasets():
    ...
    """
    # Create a CKAN API client
    print(f"ckan: {ckan_url} {ckan_key}")
...
class AuroralIntegrationPlugin(plugins.SingletonPlugin):
    ...
    def after_load(*args):
        #Inicio el hilo:
        threading.Thread(target=executeUpdate).start()
        print("The plug in is loaded")
...
def executeUpdate():
    ...
    commId = get_comunities()
    sleep_time = random.randint(1, 10)
    print('Sleeps times:'+str(sleep_time))
